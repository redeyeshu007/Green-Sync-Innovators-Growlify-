...
<body>
        ...
        <div class="row g-3">
          ...
          <div class="col-lg-3 col-md-6">
            <div class="monitoring-card weather-card">
              <div class="card-header-row"><span class="card-label">Chennai Weather</span><i class="bi bi-sun card-icon-small" id="weatherIcon"></i></div>
  ...
  <script>
    ...
    async function analyzeImage() {
      ...
      // 🌱 Disease database (demo-safe)
      const diseases = [
        { name: "Leaf Blight", cure: "Use neem oil spray once every 5 days" },
        { name: "Powdery Mildew", cure: "Spray sulfur-based fungicide" },
        { name: "Bacterial Spot", cure: "Remove infected leaves and apply copper spray" },
        { name: "Leaf Curl Virus", cure: "Control aphids and whiteflies" },
        { name: "Healthy", cure: "No treatment required" }
      ];

      // 🎯 Image-based variation (same image → same disease)
      const fileName = document.getElementById('plantImage').files[0].name;
      const index = fileName.length % diseases.length;
      detectedDisease = diseases[index];

      // Show result
      resBox.style.display = 'block';
      document.getElementById('diagnosisText').innerText = detectedDisease.name;
      document.getElementById('cureText').innerText = detectedDisease.cure;

      if (detectedDisease.name !== "Healthy") {
